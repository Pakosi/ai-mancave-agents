(function (root) {
  ...
  function getHQCanvasSceneLayout() {
    return {
      width: 480,
      height: 270,
    };
  ...
  function getHQCanvasStationFrames(layout = getHQCanvasSceneLayout()) {
    const scaleX = layout.width / 100;
    const scaleY = layout.height / 100;
    const frames = {
      "command-desk": { x: 50, y: 64, w: 29, h: 16 },
      "research-library": { x: 17, y: 31, w: 24, h: 18 },
      "builder-workstation": { x: 31, y: 67, w: 22, h: 16 },
      "analyst-desk": { x: 69, y: 67, w: 22, h: 16 },
      "automation-station": { x: 50, y: 31, w: 22, h: 16 },
      "trading-desk": { x: 83, y: 54, w: 21, h: 16 },
      "brainstorm-lounge": { x: 28, y: 49, w: 24, h: 16 },
    };
    const zones = getHQWorkZones();

    return zones.map((zone) => {
      const frame = frames[zone.id] || frames["command-desk"];
      const x = Math.round(frame.x * scaleX);
      const y = Math.round(frame.y * scaleY);
      const w = Math.round(frame.w * scaleX);
      const h = Math.round(frame.h * scaleY);
  ...
  function getHQCanvasAgentOffset(stationId, agentId, orderIndex = 0, count = 1) {
    const patterns = [
      { x: 0, y: 0 },
      { x: -8, y: 0 },
      { x: 8, y: 0 },
    ...
    const patternIndex = (orderIndex + Math.max(0, count - 1)) % patterns.length;
    return patterns[patternIndex];
  }

  function getHQCanvasTextBoxWidth(ctx, text, padding = 6, maxWidth = 140) {
    const measured = ctx.measureText(text).width + padding * 2;
    return Math.max(28, Math.min(maxWidth, Math.ceil(measured)));
  }

  function buildHQCanvasScene(options = {}) {
    const layout = options.layout || getHQCanvasSceneLayout();
    const agents = Array.isArray(options.agents) ? options.agents : [];
    const motions = Array.isArray(options.motions) ? options.motions : [];
    ...
    [...messages].forEach((message) => {
      if (message && message.role === "agent" && message.agentId) {
        latestMessages.set(message.agentId, message);
      }
    });

    const stations = getHQCanvasStationFrames(layout);
    const stationById = new Map(stations.map((station) => [station.id, station]));
    const stationBuckets = new Map();

    motions.forEach((entry) => {
      const stationId = (entry.motion && entry.motion.stationId) || "";
      if (!stationId) {
        return;
      }

      const bucket = stationBuckets.get(stationId) || [];
      bucket.push(entry);
      stationBuckets.set(stationId, bucket);
    });

    const agentsOnScene = motions.map((entry, index) => {
      const agent = agentsById.get(entry.agentId) || { id: entry.agentId, name: entry.agentId, role: "", expertise: [] };
      const display = getVisibleAgentDisplay(agent);
      const motion = entry.motion || {};
      const baseX = Math.round((parsePercent(motion.left) / 100) * layout.width);
      const baseY = Math.round((parsePercent(motion.top) / 100) * layout.height);
      const bucket = stationBuckets.get(motion.stationId || "") || [];
      const bucketIndex = Math.max(0, bucket.findIndex((item) => item.agentId === entry.agentId));
      const offset = getHQCanvasAgentOffset(motion.stationId || "", entry.agentId, bucketIndex >= 0 ? bucketIndex : index, bucket.length || 1);
      const latestMessage = latestMessages.get(entry.agentId);
      const station = stationById.get(motion.stationId) || stationById.get("command-desk");
      ...
      return {
        ...
        name,
        role: display.role || "",
        label: display.label || entry.agentId.toUpperCase(),
        x: baseX + offset.x,
        y: baseY + offset.y,
        stationId: motion.stationId || "",
        stationLabel: motion.stationLabel || station.label,
        stationPurpose: motion.stationPurpose || station.purpose,
        routeReason: motion.routeReason || "",
        latestMessage: entry.isSpeaking ? "thinking..." : (latestMessage ? latestMessage.message : (entry.roomAgent && entry.roomAgent.latestMessage) || "Thinking..."),
        appearance: getAgentCharacterStyle(agent),
        isSpeaking: Boolean(entry.isSpeaking),
        isWalking: Boolean(motion.isWalking),
        isCheckingIn: Boolean(motion.isCheckingIn),
        isWorkingAtStation: Boolean(motion.isWorkingAtStation),
        motionIntensity: Number(motion.motionIntensity || motion.activityLevel || 0),
        zIndex: Number(motion.zIndex || 0),
        title: `${name} · ${motion.stationLabel || station.label} · ${motion.stationPurpose || station.purpose}${motion.routeReason ? ` · ${motion.routeReason}` : ""}`,
      };
    }).sort((first, second) => first.zIndex - second.zIndex);

    const events = getHQInteractionEvents(motions, agents, {
      phase,
      now,
    }).slice(0, 5).map((event, index) => {
      const station = stationById.get(event.stationId) || stationById.get("command-desk");
      const x = station ? station.cx : layout.width / 2;
      const y = station ? station.y + 8 + (index * 5) : 12 + (index * 5);
    ...
    const speechBubbles = agentsOnScene.map((agent) => ({
      agentId: agent.id,
      x: agent.x,
      y: agent.y - 20,
      text: agent.latestMessage,
      isThinking: agent.isSpeaking,
      maxWidth: agent.label === "CEO" ? 132 : 118,
      title: agent.title,
    }));
    ...
    return {
      ...
      now,
      stations,
      agents: agentsOnScene,
      speechBubbles,
      events,
    };
  }

  function drawHQCanvasScene(canvas, scene = {}) {
    if (!canvas || typeof canvas.getContext !== "function") {
      return null;
    }

    const width = scene.width || getHQCanvasSceneLayout().width;
    const height = scene.height || getHQCanvasSceneLayout().height;
    ...
    const floorY = Math.round(height * 0.56);
    const time = Number(scene.now || Date.now());
    const ambience = Math.sin(time / 1700) * 0.5 + 0.5;
    const tickerOffset = Math.floor((time / 40) % 48);
    const palette = {
      wall: "#140f0a",
      wallPanel: "#20150c",
      wallLine: "#5f4320",
    ...
    for (let y = 0; y < floorY; y += 14) {
      const panelShade = y % 28 === 0 ? palette.wallPanel : "#1a120b";
    ...
    for (let x = 0; x < width; x += 48) {
      ctx.fillStyle = x % 96 === 0 ? "rgba(212,168,83,0.08)" : "rgba(255,255,255,0.02)";
      ctx.fillRect(x, 0, 1, floorY);
    }

    const wallGlows = [
      { x: Math.round(width * 0.16), y: Math.round(height * 0.18), color: "rgba(212,168,83,0.12)" },
      { x: Math.round(width * 0.52), y: Math.round(height * 0.14), color: "rgba(140,192,255,0.10)" },
      { x: Math.round(width * 0.82), y: Math.round(height * 0.20), color: "rgba(127,216,239,0.08)" },
    ];

    wallGlows.forEach((glow) => {
      ctx.fillStyle = glow.color;
      ctx.fillRect(glow.x - 10, glow.y - 10, 20, 20);
    });

    for (let x = 0; x < width; x += 16) {
      ctx.fillStyle = x % 32 === 0 ? palette.floorDark : palette.floorLight;
      ctx.fillRect(x, floorY, 16, height - floorY);
    }

    for (let y = floorY; y < height; y += 12) {
    ...
    function drawPanel(frame, fill, accent, labelColor) {
      ctx.fillStyle = fill;
      ctx.fillRect(frame.x, frame.y, frame.w, frame.h);
      ctx.fillStyle = "rgba(255,255,255,0.06)";
      ctx.fillRect(frame.x + 1, frame.y + 1, frame.w - 2, 2);
      ctx.strokeStyle = "rgba(0,0,0,0.5)";
      ctx.strokeRect(frame.x, frame.y, frame.w, frame.h);
      ctx.fillStyle = accent;
      ctx.fillRect(frame.x + 1, frame.y + frame.h - 3, frame.w - 2, 2);
      ctx.fillStyle = labelColor;
      ctx.font = "7px ui-monospace, monospace";
      ctx.textBaseline = "top";
      ctx.fillText(frame.label.toUpperCase(), frame.x + 3, frame.y + 3);
      ctx.fillStyle = "rgba(0,0,0,0.35)";
      ctx.fillRect(frame.x + 2, frame.y + frame.h - 9, Math.max(8, frame.w - 4), 6);
    }

    scene.stations.forEach((station) => {
      const fill = station.id === "command-desk"
        ? "#25180d"
        : station.id === "automation-station"
          ? "#121a27"
          : station.id === "trading-desk"
            ? "#24150b"
            : station.id === "research-library"
              ? "#132129"
              : station.id === "builder-workstation"
                ? "#22120f"
                : station.id === "analyst-desk"
                  ? "#171f27"
                  : "#1c1321";
      const accent = station.id === "command-desk"
        ? palette.gold
        : station.id === "automation-station"
          ? palette.blue
          : station.id === "trading-desk"
            ...
            : station.id === "research-library"
              ...
              : station.id === "builder-workstation"
                ...
                : station.id === "analyst-desk"
                  ? "#a4b4c5"
                  : palette.purple;
      drawPanel(station, fill, accent, palette.text);

      const labelWidth = getHQCanvasTextBoxWidth(ctx, station.name, 5, 122);
      ctx.fillStyle = "rgba(10, 7, 4, 0.86)";
      ctx.fillRect(station.x + 3, station.y + station.h + 4, labelWidth, 11);
      ctx.strokeStyle = "rgba(212,168,83,0.35)";
      ctx.strokeRect(station.x + 3, station.y + station.h + 4, labelWidth, 11);
      ctx.fillStyle = palette.text;
      ctx.font = "7px ui-monospace, monospace";
      ctx.fillText(station.name, station.x + 6, station.y + station.h + 6);

      const detail = station.purpose.length > 28 ? `${station.purpose.slice(0, 28)}...` : station.purpose;
      ctx.fillStyle = "rgba(245,231,199,0.56)";
      ctx.font = "6px ui-monospace, monospace";
      ctx.fillText(detail, station.x + 5, station.y + station.h + 17);
    });

    ctx.fillStyle = "rgba(0,0,0,0.22)";
    ctx.fillRect(0, floorY - 4, width, 4);

    const agentSprites = scene.agents || [];
    agentSprites.forEach((agent, index) => {
      const spriteX = Math.round(agent.x);
      const spriteY = Math.round(agent.y);
      const bob = Math.round(Math.sin((time / (agent.isWalking ? 120 : 220)) + index) * (agent.isWalking ? 2 : 1));
      const hop = agent.isCheckingIn ? -2 : 0;
      const baseY = spriteY + bob + hop;
      const shadowWidth = agent.isWalking ? 17 : 14;
      const shadowAlpha = agent.motionIntensity >= 0.65 ? 0.26 : 0.18;

      ctx.fillStyle = `rgba(0,0,0,${shadowAlpha})`;
      ctx.fillRect(spriteX - Math.floor(shadowWidth / 2), baseY + 16, shadowWidth, 3);

      if (agent.isCheckingIn) {
        ctx.fillStyle = agent.appearance.accent;
        ctx.fillRect(spriteX - 10, baseY - 16, 20, 20);
      }

      ctx.fillStyle = `rgba(0,0,0,0.28)`;
      ctx.fillRect(spriteX - 2, baseY - 2, 4, 18);

      const legSwing = agent.isWalking ? Math.sin((time / 120) + index) * 1 : 0;
      const armSwing = agent.isWalking ? Math.cos((time / 100) + index) * 1 : 0;
      const breath = Math.sin((time / 420) + index) * 1;
      const torsoColor = agent.appearance.torso;
      const trimColor = agent.appearance.trim;
      const accentColor = agent.appearance.accent;
      const headColor = agent.appearance.head;
      const shadowColor = agent.appearance.shadow;

      ctx.fillStyle = shadowColor;
      ctx.fillRect(spriteX - 6, baseY - 2, 12, 4);

      ctx.fillStyle = headColor;
      ctx.fillRect(spriteX - 4, baseY - 12, 8, 6);
      ctx.fillStyle = trimColor;
      ctx.fillRect(spriteX - 4, baseY - 13, 8, 1);
      ctx.fillStyle = "rgba(0,0,0,0.25)";
      ctx.fillRect(spriteX - 2, baseY - 10, 1, 1);
      ctx.fillRect(spriteX + 1, baseY - 10, 1, 1);

      ctx.fillStyle = accentColor;
      ctx.fillRect(spriteX - 5, baseY - 6, 10, 1);

      ctx.fillStyle = torsoColor;
      ctx.fillRect(spriteX - 5, baseY - 5 + Math.round(breath), 10, 8);
      ctx.fillStyle = trimColor;
      ctx.fillRect(spriteX - 5, baseY - 1 + Math.round(breath), 10, 1);
      ctx.fillRect(spriteX - 4, baseY + 2 + Math.round(breath), 8, 1);

      ctx.fillStyle = shadowColor;
      ctx.fillRect(spriteX - 7, baseY - 4 + Math.round(breath), 2, 7);
      ctx.fillRect(spriteX + 5, baseY - 4 + Math.round(breath), 2, 7);
      ctx.fillStyle = accentColor;
      ctx.fillRect(spriteX - 8, baseY - 3 + Math.round(breath) + armSwing, 2, 5);
      ctx.fillRect(spriteX + 6, baseY - 3 + Math.round(breath) - armSwing, 2, 5);

      ctx.fillStyle = accentColor;
      ctx.fillRect(spriteX - 5, baseY + 3, 4, 7);
      ctx.fillRect(spriteX + 1, baseY + 3, 4, 7);
      ctx.fillStyle = shadowColor;
      ctx.fillRect(spriteX - 5, baseY + 9, 4, 1);
      ctx.fillRect(spriteX + 1, baseY + 9, 4, 1);
      ctx.fillStyle = agent.appearance.trim;
      ctx.fillRect(spriteX - 5, baseY + 8 + legSwing, 4, 1);
      ctx.fillRect(spriteX + 1, baseY + 8 - legSwing, 4, 1);

      ctx.fillStyle = agent.isSpeaking ? "rgba(255,255,255,0.28)" : `rgba(255,255,255,${agent.motionIntensity > 0.6 ? 0.12 : 0.06})`;
      ctx.fillRect(spriteX - 9, baseY - 15, 18, 1);

      ctx.fillStyle = "rgba(0,0,0,0.55)";
      const nameBoxWidth = getHQCanvasTextBoxWidth(ctx, agent.name, 4, 112);
      ctx.fillRect(spriteX - Math.floor(nameBoxWidth / 2), baseY + 18, nameBoxWidth, 10);
      ctx.strokeStyle = `rgba(212,168,83,${agent.isCheckingIn ? 0.45 : 0.24})`;
      ctx.strokeRect(spriteX - Math.floor(nameBoxWidth / 2), baseY + 18, nameBoxWidth, 10);
      ctx.fillStyle = palette.text;
      ctx.font = "6px ui-monospace, monospace";
      ctx.fillText(agent.label || agent.name, spriteX - Math.floor(nameBoxWidth / 2) + 4, baseY + 20);
    });

    scene.events.forEach((event) => {
      const widthHint = Math.min(112, Math.max(56, event.text.length * 5 + 10));
      const x = Math.max(4, Math.min(width - widthHint - 4, event.x - Math.floor(widthHint / 2)));
      const y = Math.max(12, Math.min(height - 18, event.y - 4));
      ctx.fillStyle = "rgba(8,5,2,0.9)";
      ctx.fillRect(x, y, widthHint, 12);
      ctx.strokeStyle = event.kind === "check-in"
        ? "rgba(212,168,83,0.45)"
        : event.kind === "review"
          ? "rgba(164,180,197,0.4)"
          : "rgba(127,216,239,0.38)";
      ctx.strokeRect(x, y, widthHint, 12);
      ctx.fillStyle = palette.text;
      ctx.font = "6px ui-monospace, monospace";
      ctx.fillText(event.text, x + 4, y + 2);
    });

    ctx.fillStyle = `rgba(0,0,0,${0.18 + (0.05 * ambience)})`;
    ctx.fillRect(0, 0, width, 2);
    ctx.fillRect(0, height - 3, width, 3);

    for (let x = 0; x < width; x += 24) {
      ctx.fillStyle = x % 48 === tickerOffset % 48 ? "rgba(212,168,83,0.14)" : "rgba(255,255,255,0.03)";
  ...
  const api = {
    ...
    getHQCanvasSceneLayout,
    getHQCanvasStationFrames,
    getHQCanvasAgentOffset,
    buildHQCanvasScene,
    drawHQCanvasScene,
    getHQWorkZones,
    getHQZoneMetadata,
