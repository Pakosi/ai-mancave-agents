...
test("getHQCanvasSceneLayout and station frames provide a pixel-room map", () => {
  const layout = getHQCanvasSceneLayout();
  ...
  assert.equal(stations.length >= 7, true);
  assert.equal(stations.find((station) => station.id === "command-desk").label, "CEO Command Desk");
  assert.equal(stations.find((station) => station.id === "trading-desk").cx > 0, true);
});

test("buildHQCanvasScene separates same-station agents and adds speech bubbles", () => {
  const scene = buildHQCanvasScene({
    agents: [
      { id: "builder", name: "Builder Agent", role: "Builder", expertise: ["implementation"] },
      { id: "assistant", name: "AI Automation Agent", role: "Automation", expertise: ["automation"] },
    ],
    motions: [
      {
        agentId: "builder",
        ...
        motion: {
          ...
          stationPurpose: "Implementation, fixes, and prototype work",
      ...
      },
      {
        ...
        motion: {
          ...
          stationLabel: "Builder Workstation",
          stationPurpose: "Implementation, fixes, and prototype work",
          routeReason: "home station: Automation Station",
          motionIntensity: 0.4,
          zIndex: 101,
        },
      },
    ...
    messages: [
      { role: "agent", agentId: "builder", message: "Build is on track." },
      { role: "agent", agentId: "assistant", message: "Automation check complete." },
    ],
    phase: "execute",
    now: 12345,
  });

  assert.equal(scene.agents.length, 2);
  assert.notEqual(scene.agents[0].x, scene.agents[1].x);
  assert.equal(scene.speechBubbles.length, 2);
  assert.equal(scene.events.length >= 0, true);
  assert.equal(scene.width, 480);
  assert.equal(scene.height, 270);
});

test("drawHQCanvasScene renders onto a mock canvas without throwing", () => {
  const calls = [];
  const ctx = {
  ...
  const scene = buildHQCanvasScene({
    ...
    messages: [
      { role: "agent", agentId: "builder", message: "Rendering test." },
    ],
    phase: "execute",
    now: 12345,
  });

  drawHQCanvasScene(canvas, scene);
